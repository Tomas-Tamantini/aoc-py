...
@pytest.fixture
def circuit_builder() -> CircuitBuilder:
...
def test_smallest_edges_get_connected_first(circuit_builder):
    for _ in range(10):
...
def test_circuit_builder_keeps_track_of_number_of_circuits(circuit_builder):
    while circuit_builder.num_circuits > 1:
        circuit_builder.connect_next_smallest_edge()
    last_edge = circuit_builder.most_recent_edge
    assert {last_edge.box_a, last_edge.box_b} == {
        Vector3D(216, 146, 977),
        Vector3D(117, 168, 530),
    }
